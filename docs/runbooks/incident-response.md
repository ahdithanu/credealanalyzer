...
| `MfaFailopen` | A session was issued WITHOUT the second factor its firm requires, because Duo was unreachable and that tenant fails open. **The control was not applied.** | Check Duo's status, then check who got in: `SELECT * FROM sessions WHERE mfa_factor = 'duo_failopen'`. The customer can see it too — it is in their audit log as `auth.mfa_failopen`. |
| `MfaFailed` | Second factors that were asked for and did not pass. A few is ordinary; volume is not. | Read `code`. `username_mismatch` is someone binding their own Duo result to another person's login — treat as SEV1. `unreachable` is a Duo outage. |
